...
Cluster provides the following data:
    centroids    - each value is an [x][y]
    points       - each value is an [x][y]
    assoications - each value is a [point_index][centroid_index]
    make_images  - if true images will be generated in do_cluster()
...
Cluster requires the following methods to be implemented by comsumers:
    do_iteration - does a single iterative of a cluster algorithm
...
sub make_r_image {
    ...
    $R->startR();
    $R->send($self->_make_device_cmd($file_name).
             $self->_make_plot_cmd . 
             $self->_make_points_cmd . 
             $self->_make_centroids_cmd . 
    ...
    sub _make_plot_cmd {
        ...
        my ($biggest_x,$biggest_y) = ($biggest_x + 1 , $biggest_y + 1);
    ...
    sub _make_device_cmd {
        my($self, $file_name) = @_;
        my $exec_dir = `pwd`; chop $exec_dir;
        my $save_dir = $exec_dir .= "/clustering_output";
        `mkdir $save_dir` unless -e $save_dir;
        return "png(\"$save_dir/$file_name.png\", ".
    ...
    sub _make_nodes_cmd { #x_cluster = list of cluster's x coords
        my($self, $colors) = @_;
        my($x_cluster, $y_cluster) = $self->get_clusters;
...
sub do_cluster {
    ...
    while($truth_counter != 2) {
        $self->make_r_image if $self->make_images;
...
#TODO I think that's all for Cluster
#
#now, onto Cluster::Nathan?

## END MAKE R IMAGE STUFF ##
